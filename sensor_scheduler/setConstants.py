...
#
data.update({'renm' :	            data["R_EARTH"] / data["nm2m"]})
info.update({'renm' :	            'radius of the earth in nautical miles'})
#                                   
data.update({'reft' :	            data["R_EARTH"] * 1000.0 / data["ft2m"]})
...
#                                   
data.update({'velftps' :            data["velkmps"] * 1000.0/data["ft2m"]})
...
#                                   
data.update({'degpsec' :            data["R2D"] / data["tusec"]})
info.update({'degpsec' :            'number of degrees the earth rotates through in 1 sec'})
#                                   
data.update({'radpday' :            data["twopi"] * 1.0027379095079})
...
for key in jsonFiles:
    filename = key
    with open(os.path.join(currentDir, 'sensor-scheduler' , key + '.json'), 'w') as outfile:
