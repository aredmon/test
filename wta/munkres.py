...
@author: larry.gariepy

Module Description:
	This module contains an implementation of the Munkres optimal assignment 
	algorithm (sometimes called the Hungarian assignment algorithm, though 
	there are several variants).
	Some of the comment descriptions are cited from Bob Pilgrim's website at Murray State U., 
	http://csclab.murraystate.edu/bob.pilgrim/445/munkres.html, and this web page has a 
	visual description of how the algorithm works.    
Class Descriptions:
    -N/A
...
Inputs:
    cost_matrix - an MxN matrix of cost values for associating m new object to n existing objects
				  (detections to measurements, measurements to tracks, tracks to tracks, etc.)
Outputs: 
    star - assignment matrix of 0's and 1's where a '1' in the (i,j) entry indicates the optimal 
           solution includes the assignment of element i to element j.  Note, if m!=n, then 
           there will not be an assignment of every object.  Rather, the solution will contain 
           a number of assignments equal to min(m,n)
"""
import numpy as np
...
# 				print("Munkres: found ",m," assignments; DONE")
		...
		# #                 Go to Step 3.
			...
			k = 0
			while True:  # while last primed Z(k) has starred zero in the same column
